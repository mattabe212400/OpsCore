// ══════════════════════════════════════════════
// tasks.js
// Tasks & Goals page: renders the Kanban board (4 columns), semester goals grid,
// and KPI cards. Also contains Meeting Notes: the notes grid, note detail view,
// structured note builder (officer reports, announcements, old/new business),
// note search filter, note print function, and all note-card helpers.
// ══════════════════════════════════════════════
...
function renderNotes(){
  ...
  const tn={chapter:'Organization',exec:'Executive',committee:'Committee',judicial:'Judicial',retreat:'Retreat'};
  const notes=D.notes;
  if(!notes.length){
...
async function deleteShift(id){
  ...
  const ok=await confirmDialog('Delete Shift','Remove this safety officer shift?');
  if(!ok)return;
...
function openNoteDetail(id){
  ...
  let html=`
    <!-- Header -->
    <div class="nd-header">
      <div style="font-size:11px;color:var(--mt);margin-bottom:4px">${n.chapter||'OpsCore — Nexus Chapter'}</div>
