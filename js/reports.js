...
function rpWeekly(){
  ...
  const unassignedShifts=D.shifts.filter(s=>isUp(s.date)&&!s.memberId).length;
  ...
  rpSection('ti-calendar-event','Upcoming Events',
    upEvents.length?rpTable(['Event','Date','Type','Location','Required'],upEvents.map(e=>[
      `<strong>${e.title}</strong>`,fd(e.date),e.type||'—',e.location||'—',e.mandatory?'<span style="color:var(--rd);font-weight:600">Yes</span>':'No'
  ...
  rpSection('ti-checkbox','Overdue Tasks',
    overdue.length?rpTable(['Task','Assignee','Priority','Days Overdue'],overdue.slice(0,8).map(t=>{
      ...
      return[`<strong>${t.title}</strong>`,mB(t.assignedTo).name,`<span style="color:${pc[t.priority]||'var(--mt)'};">${t.priority}</span>`,`<span style="color:var(--rd)">${daysOv}d</span>`];
...
function rpAttendance(){
  const tot=D.members.length||1;
  const avg=Math.round(D.members.reduce((s,m)=>s+aR(m.id),0)/tot);
  const good=D.members.filter(m=>aR(m.id)>=85).length;
  const risk=D.members.filter(m=>{const r=aR(m.id);return r>=65&&r<85;}).length;
  const warn=D.members.filter(m=>aR(m.id)<65).length;
  const mandEvents=D.events.filter(e=>e.mandatory&&!isUp(e.date));
  const sorted=[...D.members].sort((a,b)=>aR(a.id)-aR(b.id));
  ...
  rpSection('ti-alert-triangle','Members Requiring Attention',
    (()=>{
      const atRisk=sorted.filter(m=>aR(m.id)<80);
  ...
  rpSection('ti-users','Full Member Attendance',
    rpTable(['Member','Class','Role','Attendance Rate','Status'],[...D.members].sort((a,b)=>aR(b.id)-aR(a.id)).map(m=>{
      const r=aR(m.id);
  ...
  rpSection('ti-calendar-check','Mandatory Events This Semester',
    mandEvents.length?rpTable(['Event','Date','Present','Absent','Rate'],mandEvents.sort((a,b)=>b.date.localeCompare(a.date)).map(ev=>{
      ...
      return[`<strong>${ev.title}</strong>`,fd(ev.date),pres,abs,`<span style="color:${rate>=85?'var(--gn)':rate>=75?'var(--navy)':'var(--rd)'};font-weight:600">${rate}%</span>`];
  ...
  rpSection('ti-users','Full Rushee List',
    rushees.length?rpTable(['Name','Stage','Bid Score','Recruiter','Last Contact'],
      [...rushees].sort((a,b)=>(b.bidScore||0)-(a.bidScore||0)).map(r=>{
        ...
        return[`<strong>${r.name}</strong>`,r.stage,`<span style="font-weight:700;color:${sc}">${r.bidScore||0}</span>`,mB(r.recruiter).name||'—',r.lastContact?fds(r.lastContact):'<span style="color:var(--rd)">Never</span>'];
  ...
  rpSection('ti-receipt','Recent Expenses',
    expenses.length?rpTable(['Description','Category','Amount','Date'],
      [...expenses].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).slice(0,10).map(e=>[
        e.desc||e.description||'—',e.category||'—',`$${(e.amount||0).toLocaleString()}`,e.date?fds(e.date):'—'
