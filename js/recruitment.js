...
function rcDrawAlerts(rushees){
  ...
  const nextEv=(D.recruitment.events||[]).filter(e=>isUpcoming(e.date)).sort((a,b)=>a.date.localeCompare(b.date))[0];
...
function rcRenderTable(){
  ...
  el.innerHTML=`<thead><tr><th>Name</th><th>Year</th><th>Major</th><th>Recruiter</th><th>Stage</th><th>Events</th><th>Score</th><th>Last Contact</th><th></th></tr></thead><tbody>${rushees.map(r=>{
    const recName=r.recruiter?getMember(r.recruiter).name.split(' ')[0]:'—';
      ...
      <div class="rc-col-body rc-col-drop" id="kd-${stage.replace(/ /g,'_')}"
        ...
        ${rushees.map(r=>`<div class="rc-card" draggable="true" id="kc-${r.id}"
          ...
          <div style="display:flex;align-items:center;justify-content:space-between">
            <div style="font-size:10px;color:var(--ht)">${r.eventsAttended} event${r.eventsAttended!==1?'s':''} · ${r.recruiter?getMember(r.recruiter).name.split(' ')[0]:'Unassigned'}</div>
...
function rcOpenProfile(id){
  ...
  const recName=r.recruiter?getMember(r.recruiter).name:'Unassigned';
  const daysSince=r.lastContact?Math.round((new Date()-new Date(r.lastContact+'T12:00:00'))/86400000):null;
  const rcEvents=(D.recruitment.events||[]).filter(e=>!isUpcoming(e.date)).slice(0,r.eventsAttended);
  ...
  document.getElementById('rcp-body').innerHTML=`
    ...
    <div style="margin-bottom:14px">
      ...
      ${rcEvents.length?rcEvents.map(e=>`<div class="d-feed-row"><div class="d-feed-av" style="background:var(--bl-bg);color:var(--bl)"><i class="ti ti-calendar-check" style="font-size:11px"></i></div><div style="flex:1"><div style="font-size:11.5px;font-weight:500">${e.name}</div><div style="font-size:10px;color:var(--ht)">${formatDate(e.date)} · ${e.type}</div></div></div>`).join(''):`<div style="font-size:11.5px;color:var(--ht)">No events attended yet</div>`}
    </div>
    <!-- Notes -->
    <div>
      ...
      <div id="rcp-notes">${(r.notes||[]).length?(r.notes||[]).slice().reverse().map(n=>`<div class="rc-note" style="margin-bottom:7px"><div class="rc-note-meta"><span>${getMember(n.by).name}</span><span>${formatDateShort(n.date)}</span></div>${n.text}</div>`).join(''):`<div style="font-size:11.5px;color:var(--ht)">No notes yet — add your first observation.</div>`}
      </div>
    </div>`;
...
function rcAddNote(rusheeId){
  ...
  r.lastContact=new Date().toISOString().split('T')[0];
  saveData();rcOpenProfile(rusheeId);toast('Note added','success');
}
...
function rcAddTag(rusheeId){
  ...
  if(!r.tags)r.tags=[];
  r.tags.push(tag);saveData();rcOpenProfile(rusheeId);
}
...
function rcAddRushee(){
  ...
  saveData();closeM(null,document.getElementById('m-rc-add'));
  if(RC_ACTIVE_TAB==='rc-rushees')rcRenderTable();
...
function rcAddEvent(){
  ...
  saveData();closeM(null,document.getElementById('m-rc-event'));
