...
function renderMembers(){
  ...
  document.getElementById('m-table').innerHTML=`<thead><tr><th>Member</th><th>Grad Year</th><th>Class</th><th>Role</th><th>Attendance</th><th>Live-in</th><th>Engagement</th><th></th></tr></thead><tbody>${D.members.map(m=>{const r=getAttendanceRate(m.id);const dots=[...Array(5)].map((_,i)=>`<div class="edot ${i<Math.round(r/20)?'f':'e'}"></div>`).join('');return`<tr><td><div style="display:flex;align-items:center;gap:7px"><div class="sh-av" style="width:25px;height:25px;font-size:8.5px">${m.initials}</div><span style="font-weight:500">${m.name}</span></div></td><td style="color:var(--mt)">${m.year}</td><td>${m.classYear}</td><td><span class="badge ${m.role!=='Member'?'bb2':'bm2'}">${m.role}</span></td><td style="font-weight:500;color:${r>=85?'var(--gn)':r>=75?'var(--navy)':r>=65?'var(--am)':'var(--rd)'}">${r}%</td><td>${m.liveIn?'Yes':'—'}</td><td><div class="edots">${dots}</div></td><td><button class="btn" style="height:23px;font-size:10.5px" onclick="openEditMember('${m.id}')"><i class="ti ti-pencil"></i></button></td></tr>`;}).join('')}</tbody>`;
}

function renderCommittees(){
  document.getElementById('co-cnt').textContent=D.committees.length+' committees this semester';
  document.getElementById('co-grid').innerHTML=D.committees.map(c=>`<div class="card" style="cursor:pointer;transition:border-color .1s" onmouseover="this.style.borderColor='var(--navy)'" onmouseout="this.style.borderColor='var(--bdr)'"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:9px"><span style="font-size:13px;font-weight:500">${c.name}</span><div style="display:flex;align-items:center;gap:6px"><span style="font-size:10.5px;color:var(--mt)">${(c.members||[]).length} members</span><button class="btn" style="height:22px;font-size:10px;padding:0 7px" onclick="openEditComm('${c.id}')"><i class="ti ti-pencil"></i></button></div></div><p style="font-size:11.5px;color:var(--mt);line-height:1.5;margin-bottom:9px">${c.desc||'No description'}</p><div style="font-size:11px;color:var(--ht);border-top:1px solid var(--bdr);padding-top:8px;display:flex;justify-content:space-between"><span>Chair:</span><span style="font-weight:500;color:var(--tx)">${c.chair?getMember(c.chair).name:'Unassigned'}</span></div></div>`).join('')||`<div style="grid-column:1/-1">${es('ti-sitemap','blue','No committees','Create committees to organize chapter working groups.',`<button class="btn btn-p" onclick="openM('m-addcomm')"><i class="ti ti-plus"></i>New Committee</button>`)}</div>`;
}

function renderAnalytics(){
  const tot=D.members.length;
  const avg=tot?Math.round(D.members.reduce((s,m)=>s+getAttendanceRate(m.id),0)/tot):0;
  const dn=D.tasks.filter(t=>t.status==='done').length;
  const ip=D.tasks.filter(t=>t.status==='in_progress').length;
  const ov=D.tasks.filter(t=>isOverdue(t.dueDate)&&t.status!=='done').length;
  const td=D.tasks.filter(t=>t.status==='todo'&&!isOverdue(t.dueDate)).length;
  const onT=D.goals.filter(g=>percent(g.current,g.target)>=50).length;
  const openCases=D.cases.filter(c=>!['resolved','dismissed'].includes(c.status)).length;

  // ── KPIs ──
  document.getElementById('an-kpi').innerHTML=
    kpi('Chapter Attendance',avg+'%',avg>=85?'Above target':'Below 85% target',avg>=85?'up':'down')+
  ...
  const hi=D.members.filter(m=>getAttendanceRate(m.id)>=90).length;
  const go2=D.members.filter(m=>{const r=getAttendanceRate(m.id);return r>=75&&r<90;}).length;
  const ri=D.members.filter(m=>{const r=getAttendanceRate(m.id);return r>=60&&r<75;}).length;
  const lo=D.members.filter(m=>getAttendanceRate(m.id)<60).length;
  ...
  document.getElementById('an-goals').innerHTML=D.goals.map(g=>{
    const p=Math.min(percent(g.current,g.target),100);
    return`<div><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-size:12px;font-weight:500">${g.title}</span><span style="font-size:11px;color:var(--mt)">${g.current}/${g.target} ${g.unit}</span></div><div class="pb" style="height:5px"><div class="pf" style="width:${p}%;background:${progressColor(p)}"></div></div></div>`;
...
function anDrawLine(){
  ...
  if(mandPast.length>=2){
    points=mandPast.slice(-10).map(ev=>{
      ...
      return{label:monthShort(ev.date)+' '+dayOfMonth(ev.date),val:Math.round(pres/tot*100)};
    });
  }
...
function anDrawHealth(avgAtt,dnTasks,openCases){
  ...
  const upcomingEvt=D.events.filter(e=>isUpcoming(e.date)).length;
  ...
  const items=[
    ...
    {label:'J-Board Status',val:caseScore,target:90,icon:'ti-scale',color:caseScore>=85?'var(--gn)':'var(--rd)'},
  ];

  const overallScore=Math.round(items.reduce((s,x)=>s+(x.val*{Attendance:.30,'Task Completion':.25,'GPA Score':.20,'Event Activity':.15,'J-Board Status':.10}[x.label]),0));
...
function anDrawOfficers(){
  ...
  html+=officers.map(m=>{
    const att=getAttendanceRate(m.id);
    const ts=getTaskMetrics(m.id);
