...
function renderSober(){
  ...
  const emptyRow=`<tr><td colspan="${ro?5:6}"><div style="padding:8px">${es('ti-shield-check','green','No upcoming sober shifts','Add shifts for upcoming events to track coverage.',emptyAddBtn)}</div></td></tr>`;
...
function renderMembers(){
  ...
  document.getElementById('m-table').innerHTML=`<thead><tr><th>Member</th><th>Grad Year</th><th>Class</th><th>Role</th><th>Attendance</th><th>Live-in</th><th>Engagement</th>${editCol}</tr></thead><tbody>${D.members.map(m=>{const r=getAttendanceRate(m.id);const dots=[...Array(5)].map((_,i)=>`<div class="edot ${i<Math.round(r/20)?'f':'e'}"></div>`).join('');const editCell=canEdit?`<td><button class="btn" style="height:23px;font-size:10.5px" onclick="openEditMember('${m.id}')"><i class="ti ti-pencil"></i></button></td>`:'';return`<tr><td><div style="display:flex;align-items:center;gap:7px"><div class="sh-av" style="width:25px;height:25px;font-size:8.5px">${m.initials}</div><span style="font-weight:500">${m.name}</span></div></td><td style="color:var(--mt)">${m.year}</td><td>${m.classYear}</td><td><span class="badge ${m.role!=='Member'?'bb2':'bm2'}">${m.role}</span></td><td style="font-weight:500;color:${r>=85?'var(--gn)':r>=75?'var(--navy)':r>=65?'var(--am)':'var(--rd)'}">${r}%</td><td>${m.liveIn?'Yes':'—'}</td><td><div class="edots">${dots}</div></td>${editCell}</tr>`;}).join('')}</tbody>`;
}
