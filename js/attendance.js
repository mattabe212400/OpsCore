...
function renderAttendance(){
  ...
  document.getElementById('a-kpi').innerHTML=kpi('Semester avg',avg+'%',getSemester(),avg>=85?'up':'down')+kpi('Excused absences',excused,'Semester total','neutral')+kpi('Unexcused',absent,'Semester total',absent>20?'down':'neutral')+kpi('Warnings issued',D.members.filter(m=>aR(m.id)<75).length,'Below 75%',D.members.filter(m=>aR(m.id)<75).length>0?'down':'neutral');
...
function attDrawTrend(){
  ...
  const pts=mandPast.slice(-12).map(ev=>{
    ...
    return{label:mos(ev.date)+' '+dom(ev.date),val:Math.round(pres/tot*100),title:ev.title};
  });
...
function attDrawDonut(){
  ...
  const counts=tiers.map(t=>({...t,n:D.members.filter(m=>{const r=aR(m.id);return r>=t.min&&(t.min===0||true)&&r< (tiers[tiers.indexOf(t)-1]?.min||101)}).length}));
  // Recompute cleanly
  const good=D.members.filter(m=>aR(m.id)>=85).length;
  const onT=D.members.filter(m=>{const r=aR(m.id);return r>=75&&r<85;}).length;
  const risk=D.members.filter(m=>{const r=aR(m.id);return r>=65&&r<75;}).length;
...
function attDrawEventBars(){
  ...
  el.innerHTML=mandPast.map(ev=>{
    ...
    return`<div class="att-ev-bar">
      <span class="att-ev-label" title="${ev.title} · ${fd(ev.date)}">${ev.title}</span>
...
function attDrawRiskGrid(){
  ...
  const riskCount=D.members.filter(m=>aR(m.id)<85).length;
  if(sumEl)sumEl.textContent=riskCount>0?`${riskCount} member${riskCount>1?'s':''} below 85% target`:'All members above 85%';

  el.innerHTML=tiers.map(t=>{
    const mems=D.members.filter(m=>{const r=aR(m.id);return r>=t.min&&r<t.max;}).sort((a,b)=>aR(a.id)-aR(b.id));
