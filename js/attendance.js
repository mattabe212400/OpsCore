...
function renderAttendance(){
  ...
  document.getElementById('a-kpi').innerHTML=kpi('Semester avg',avg+'%',getSemester(),avg>=85?'up':'down')+kpi('Excused absences',excused,'Semester total','neutral')+kpi('Unexcused',absent,'Semester total',absent>20?'down':'neutral')+kpi('Warnings issued',D.members.filter(m=>getAttendanceRate(m.id)<75).length,'Below 75%',D.members.filter(m=>getAttendanceRate(m.id)<75).length>0?'down':'neutral');
...
function attDrawRiskGrid(){
  ...
  el.innerHTML=tiers.map(t=>{
    ...
    return`<div class="att-risk-tier" style="border-color:${t.border}22">
      ...
      ${mems.length?mems.slice(0,8).map(m=>{const r=getAttendanceRate(m.id);return`<div class="att-risk-member">
